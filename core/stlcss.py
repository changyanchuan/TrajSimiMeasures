...
def stlcss_opt(trajs, lens, dpmtx, eps, delta, dists):
    # trajs = [batch, 2, seq, 3]
    # traj_len = [batch, 2]
        ...
        while pid < n2:
            if pid <= it < pid + n1 :
                if squared_dist(t1[it-pid], t2[pid]) < squared_eps \
                    ...
                    cost = dpmtx[bid, pid, ll_ridx] + 1
                else:
                    cost = max(dpmtx[bid, pid, l_ridx], dpmtx[bid, pid+1, l_ridx])
                dpmtx[bid, pid+1, ridx] = cost
            pid += nthread
        cuda.syncthreads()
        
    if thid == 0:
        dists[bid] = 1 - dpmtx[bid, n2, (niters+1)%3] / min(n1, n2) # normalized value
   

def stlcss(t1: np.ndarray, len1: np.ndarray, t2: np.ndarray, len2: np.ndarray, eps, delta,
           threads_per_traj = 64):
    # t1, t2: [batch, seq, 3]
    # len1, len2: [batch]
    # eps: spatial distance threshold
    # delta: temporal distance threshold
    ...
    with cuda.defer_cleanup():
        stlcss_opt[batch_size, threads_per_traj](trajs_g, lens_g, dpmtx_g, eps, delta, dists_g)
        cuda.synchronize()
