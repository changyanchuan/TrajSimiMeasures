...
def edr_opt(trajs, lens, dpmtx, eps, dists):
    # trajs = [batch, 2, seq, 2]
    # traj_len = [batch, 2]
    ...
    for it in range(niters):
        ...
        while pid < n2:
            if pid <= it < pid + n1 :
                subcost = 0 if squared_dist(t1[it-pid], t2[pid]) < squared_eps else 1
...
def edr(t1: np.ndarray, len1: np.ndarray, t2: np.ndarray, len2: np.ndarray, eps, 
        threads_per_traj = 64):
    # t1, t2: [batch, seq, 2]
    # len1, len2: [batch]
    ...
    with cuda.defer_cleanup():
        edr_opt[batch_size, threads_per_traj](trajs_g, lens_g, dpmtx_g, eps, dists_g)
        cuda.synchronize()
