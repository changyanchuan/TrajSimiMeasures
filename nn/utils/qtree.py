...
def get_qtree_feat(root):
    ...
    while queue:
        ...
        if tree.children:
            for child in tree.children:
                vir_node_edge_list.append((hash(tree), hash(child)))
                queue.append(child)

    vir_node_num = len(virtual_node)
    for index, node in enumerate(virtual_node):
        vir_node2id[hash(node)] = index
        vir_id2center[index] = node.center

    assert vir_node2id[hash(root)] == vir_node_num - 1, "root hash 没有对应为最后一个点"
    ...
    assert word_embedding_name2id[hash(root)] == vir_node_num, f"word embedding root hash 对应有误 {word_embedding_name2id[hash(root)]},{vir_node_num}"
...
class _QuadTree(object):
    ...
    def __init__(self, x, y, width, height, max_items, max_depth, _depth=0):
        ...
        self._depth = _depth        
        # _aa = int(hex(struct.unpack('<I', struct.pack('<f', self.center[0]))[0]), 16)
        # _bb = int(hex(struct.unpack('<I', struct.pack('<f', self.center[1]))[0]), 16)
        # self.center_id = _aa*10**32+_bb
        self.center_id = metrohash.hash128_int(str(self.center))
    ...
    def _intersect_tree(self, rect, results=None, uniq_tree=None):
        ...
        for node in self.nodes:
            if node.rect[2] >= rect[0] and node.rect[0] <= rect[2] and node.rect[3] >= rect[1] and node.rect[1] <= rect[3]:
                # print(self.center)
                if hash(self) not in uniq_tree:
                    results.append(self)
                    uniq_tree.add(hash(self))
                break
        return results
    ...
    def __hash__(self):
        ...
        # return metrohash.hash128_int(str(self.center))
        # return hash( f'{self.center[0]:.10f}{self.center[1]:.10f}' ) 
        # aa = int(hex(struct.unpack('<I', struct.pack('<f', self.center[0]))[0]), 16)
        # bb = int(hex(struct.unpack('<I', struct.pack('<f', self.center[1]))[0]), 16)
        # return aa*10**32+bb
